...
def mission1():
    ...
    drone.takeoff()
    drone.move_up(60)
    drone.move_forward(FORWARD_DIS)
    rounds = qrRead()
    print(rounds)
    ...
    QR1 = rounds
    time.sleep(2)
    drone.move_left(FORWARD_LEFT)
    # วันจริงให้เพิ่มความสูงอย่าลืมด้วย
    # drone.move_up(55)
...
while True:
    if(keyboard.read_key()=='q'):
        try:
            ...
            if ask_input == 'st':
                mission1()
                ask = input('[MISSION2] If ready press y and enter: ')
                if ask == 'y':
                    mission2()
                    # ask = input('[MISSION3] If ready press y and enter: ')
                    # if ask == 'y':
                    #     mission3()
            if ask_input == 'tf':
                drone.takeoff()
